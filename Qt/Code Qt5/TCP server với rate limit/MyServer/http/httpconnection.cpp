...
void HttpConnection::processGet(QByteArray data)
{
    ...
    QString header = m_request.value("svr_header","");//k tồn tại nó trả gt default chứ kp lỗi
    header.append(data);
    qDebug() << "m_request.value(svr_header,): " << m_request.value("svr_header","");
...
void HttpConnection::handleRequest()
{
    ...
    if(fi.isDir())
    {
        ...
        QString indexFile = actualFile + "\\HTML Basic.html";
...
void HttpConnection::sendFile(QString file)
{
    ...
    rateTransfer = new RateTransfer(this);

    connect(rateTransfer,&RateTransfer::started,this,&HttpConnection::started);
    connect(rateTransfer,&RateTransfer::finished,this,&HttpConnection::finished);
    connect(rateTransfer,&RateTransfer::error,this,&HttpConnection::transferError);
    connect(rateTransfer,&RateTransfer::transfered,this,&HttpConnection::transfered);

    if(!m_file->open(QFile::ReadOnly))
    {
        qWarning() << "Could not open source file: " << file;
        m_socket->close();
    }
    rateTransfer->setSource(m_file);
    rateTransfer->setDestination(m_socket);
    rateTransfer->setRate(m_rate);
    rateTransfer->setSize(1024);

    rateTransfer->start();
    m_response.remove("code");
}

QString HttpConnection::getMimeType(QString path)//hàm lấy đuôi file
{
...
{
   qDebug() << this <<  "File transfer error: " << rateTransfer->errorString();
   m_file->close();
   m_socket->close();
}
